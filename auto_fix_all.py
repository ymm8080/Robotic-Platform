...
class AutoFixer:
    ...
    def fix_auth_py(self) -> str:
        ...
        if 'with self._lock:' not in content:
            ...
            method_pattern = (
                r'def get_valid_token\(self, client: httpx\.Client\)'
                r' -> str:.*?"""Return a valid token'
            )
                        ...
                        if lines[j].strip() == '' and j > i:
                            ...
                            if 'except FileNotFoundError:' not in func_body:
                                ...
                                func_body = func_body.replace(
                                    ...
                                    '        logger.error(f"OAuth2 client secret'
    ...
    def fix_grafana_dashboard(self) -> str:
        ...
        dashboard_path = (
            self.project_root / "monitoring" / "dashboards"
            / "v5-traffic-coordinator.json"
        )
  ...
  "templating": {
    ...
    def fix_sap_coordinator_bridge(self) -> str:
        ...
        if 'GRACE_POLLS = 2' not in content.split('\n')[:50]:
            ...
            for i, line in enumerate(lines):
                if 'AUTO_CONFIRM = os.getenv("SAP_TC_AUTO_CONFIRM", "0") == "1"' in line:
                    lines.insert(
                        i + 1,
                        '# Number of consecutive polls a task must be'
                        ' inactive before auto-confirming.',
                    )
        ...
        if 'GRACE_POLLS = 2' in content and 'if tid not in self._inactive_since:' in content:
            ...
            pattern = (
                r'# Not in any explicit list — use grace-period fallback'
                r'\s+GRACE_POLLS = 2\s+if tid not in self\._inactive_since:'
                r'\s+self\._inactive_since\[tid\] = self\._poll_count'
                r'\s+logger\.info\('
            )
            replacement = (
...
def main():
    ...
    if success:
        ...
        print("2. Commit the fixes: git commit -am 'fix: apply all AI "

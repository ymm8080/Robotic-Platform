...
# ponytail: 推导而非新增 factsheet 字段 — 避免 [GUESS] 硬编码品牌档位;
...
def _demo() -> None:
    ...
    for brand, (tier, policy) in expected.items():
        ...
        assert got_tier is tier, f"{brand}: tier {got_tier} != {tier}"
        assert got_policy is policy, f"{brand}: policy {got_policy} != {policy}"
    print(f"OK: 6 brands classified — {len(expected)} tiers resolved correctly")

...
class CatPawMonitor:
    ...
    def is_process_running(self, process_name: str, pid: int | None = None) -> bool:
    ...
    def start_process(self, process_config: dict[str, Any]) -> subprocess.Popen | None:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if return_code is not None:
                    ...
                    recent_restarts = [t for t in restart_history if t > hour_ago]
                    ...
                    if new_proc:
                        ...
                        restart_history = [
                            t for t in restart_history if t > now - timedelta(hours=24)
                        ]
                    else:
                ...
                if self.config["monitoring"]["enable_http_check"]:
                    ...
                    if health_url:
                        is_healthy = self.check_http_health(
                            health_url, process_config["health_check_timeout"]
                        )
                        if not is_healthy:
                            self.logger.warning(
    ...
    def run(self):
        ...
        for process_config in self.config["processes"]:
            thread = threading.Thread(
                target=self.monitor_process, args=(process_config,), daemon=True
            )
            thread.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--create-task", action="store_true", help="Create Windows Task Scheduler XML"
    )
    parser.add_argument(
        "--create-restart-script", action="store_true", help="Create restart script"
    )
    parser.add_argument(
        "--config", default="catpaw_monitor_config.json", help="Path to config file"
    )
    ...
    parser.add_argument(
        "--check", action="store_true", help="Check all processes and health endpoints"
    )

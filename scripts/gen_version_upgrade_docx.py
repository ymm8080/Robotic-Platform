...
def make_table(ws, headers, rows, col_widths=None):
    ...
    for i, h in enumerate(headers):
        cell = tbl.cell(0, i)
        set_cell(
            cell, h, bold=True, size=9, align=WD_ALIGN_PARAGRAPH.CENTER, color=(0xFF, 0xFF, 0xFF)
        )
...
p.add_run(
    " — 同时对接 SAP EWM（Extended Warehouse Management）和 SAP WM（Classic Warehouse Management），在同一平台统一调度机器人。"
)

doc.add_heading("后端插件注册表", level=2)
doc.add_paragraph(
    "采用与机器人策略模式完全相同的 Plugin Registry 模式，每个后端实现统一的 WarehouseBackend 接口："
)
...
rows = [
    [
        ...
        "backends/base.py",
        "抽象契约：list_tasks · get_task · create_task · confirm_task · cancel_task",
    ],
...
p.add_run(
    "config.yaml 中按仓库配置 backend 类型，同一个 sap-bridge 容器可同时对接 EWM 和 WM："
).font.size = Pt(9)
...
doc.add_paragraph(
    "EWM + WM 双系统支持已经在 v3.4 完整实现，不在升级路径中。升级中对 SAP 对接层的影响："
)
p = doc.add_paragraph()
...
add_bullet(
    p,
    "v5.0：",
    "K8s 编排，对接层作为无状态 Pod 水平扩展，config.yaml 转为 ConfigMap",
    (0xC0, 0x00, 0x00),
)
...
r = [
    [
        "v3.4 有什么问题？",
        '没问题。生产就绪，已实现 EWM + WM 双系统对接。升级是"架构演进"，不是"修复缺陷"',
    ],
    [
        "v3.4 能接 EWM 和 WM 吗？",
        "能。双后端已在 v3.4 实现：EWM 走 OData，WM 走 RFC/BAPI，通过 config.yaml 按仓库配置切换",
    ],
...
r = [
    ...
    [
        "仓库后端抽象",
        ".claude/memory/warehouse-backend-abstraction.md",
        "双 EWM+WM 后端 Plugin 设计",
    ],

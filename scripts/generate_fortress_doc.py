...
THREE_LAWS = [
    ...
    ("🔒 铁律一：治理层——零信任博弈", "任何跨边界的数据默认脏，任何参与者默认是机会主义者。", "交叉验证状态（电量 vs 功耗）、信誉度加权调度、防御性编程（假设下游崩溃）。"),

    ("🛡️ 铁律二：物理层——熵增冗余", "逻辑追求最优，物理必须为最坏买单。", "快慢车道物理隔离、时空硬边界（资源生存锁。"),

    ("⚖️ 铁律三：生存层——因果存证", "事故后 5 分钟内无法还原因果链，系统在法律上非法。", "WORM 黑匣子（不可篡改）、全链路因果图谱、降级生存权（断网本地安全停车）。")
...
TECH_SPECS = [
    ...
    ("动态安全公式", "S = V × K_brake + RTT × V + C_static。约束：T_min 绝对不低于 1.5s（硬下限）。"),

    ("DDS 加密损耗", "加密增加约 15% RTT，已在公式中预留容限。"),

    ("时间同步", "默认 NTP (<10ms)。动态避障场景必须升级 PTP (<1ms)。"),

    ("经济模型 (RaaS)", "Task Allocator 效用函数预留 gamma 权重（成本因子）。当前 gamma=0.0，未来配置启用。"),

    ("隐私合规", "数据最小化。视觉数据本地去特征化处理，仅上传骨骼点；日志中人员 ID 使用 Hash 脱敏。")
...
def create_word():
    ...
    for i, (idx, cat, name, patch) in enumerate(RHINOS):
        ...
        elif cat == '物理': bg = 'FFD966'

        elif cat == '运维': bg = 'C5E0B4'
    ...
    for m in MONITORING: doc.add_paragraph(m, style='List Bullet')
    ...
    doc.add_paragraph('2. Zone Lockdown：若无效，选中区域冻结，发送 Level 0 E-Stop。', style='List Number')

    doc.add_paragraph('3. 上帝手柄：使用 Wifi Direct 直连底盘，绕过网络物理接管。', style='List Number')



    doc.add_heading('5.6 SOP-E：网络分区与脑裂', 2)

    doc.add_paragraph('1. 客户端降级：心跳丢失 >5s，切换离线安全模式（停车）。', style='List Number')
    ...
    for item in GROUND_TRUTH: doc.add_paragraph(f"- {item}", style='List Bullet')

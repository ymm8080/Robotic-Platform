...
These skills are **synced** to Cursor rules format (`.cursor/rules/*.mdc`) for automatic context injection:

| Skill | Cursor Rule | Globs |
|-------|-------------|-------|
| `SAP_OData_Handler.md` | `sap-odata-handler.mdc` | `sap-bridge/**` |
| `VDA5050_State_Machine.md` | `vda5050-state-machine.mdc` | `robot/**`, `agv/**`, `state_machine/**`, `mqtt/**` |
| `Async_Retry_Tester.md` | `async-retry-tester.mdc` | `tests/**`, `sap-bridge/test_*`, `mqtt/test_*` |
| `state-machine/SKILL.md` | `ewm-state-machine.mdc` | `state_machine/**`, `mqtt/**` |
| `dify-workflow.md` | `dify-workflow.mdc` | `dify/**`, `workflows/**` |

Cursor rules auto-trigger when editing matching files (via `globs`). Claude Code skills are invoked via `/skill-name`.

...
def run(cmd, check=True, cwd=None):
    ...
    if result.stderr: print(result.stderr[:3000], file=sys.stderr)
...
run(f"git fetch origin master", cwd=CWD)
...
# 3. Clear pycache
for root, dirs, files in os.walk(os.path.join(CWD, "sap-bridge")):
    for d in dirs:
        if d == "__pycache__":
...
if "def _parse_response(" in content:
    ...
    test_content = test_content.replace("ZewmRobcoClient.parse_response", "ZewmRobcoClient._parse_response")
elif "def parse_response(" in content:
    # Client uses parse_response
    test_content = test_content.replace("ZewmRobcoClient._parse_response", "ZewmRobcoClient.parse_response")
...
# 7. Run ruff
run("python -m ruff check --fix sap-bridge/auth.py sap-bridge/clients/ sap-bridge/tests/test_zewm_robco_client.py", check=False, cwd=CWD)
print("=== Ruff done ===")

# 8. Run tests
test_result = run("python -m pytest sap-bridge/tests/test_zewm_robco_client.py -v --tb=short", check=False, cwd=CWD)
print(f"=== Tests exit code: {test_result.returncode} ===")

# 9. Commit and push
run("git add -A", cwd=CWD)
run('git commit -m "fix: merge conflicts resolved, AI review fixes, test compatibility"', check=False, cwd=CWD)

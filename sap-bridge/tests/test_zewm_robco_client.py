...
=======
    def test_default_base_url_no_error(self):
        """Default base_url logs info but does not produce validation errors."""
>>>>>>> cc689e2268ba4a3d37c8aff7f99fb9dd46e122da
        errs = ZewmRobcoClient.validate_config({
...
=======
        assert not any("base_url" in e.lower() for e in errs)

    def test_default_client_no_error(self):
        """Default SAP client (100) logs info but does not produce errors."""
        errs = ZewmRobcoClient.validate_config({
        ...
        assert not any("client" in e.lower() for e in errs)
>>>>>>> cc689e2268ba4a3d37c8aff7f99fb9dd46e122da
...
# ========================================================================
# 11. OAuth2 mode -- initialization, token handling
# ========================================================================


@pytest.fixture
def oauth2_config():
    """OAuth2 configuration dict for ZewmRobcoClient."""
    return {
        "enabled": True,
        "base_url": "https://sap-test.example.com:443",
        "client": "100",
        "auth_mode": "oauth2",
        "oauth2": {
            "token_url": "https://sap-test.example.com:44300/sap/bc/sec/oauth2/token",
...
@pytest.fixture
def oauth2_client(oauth2_config, mock_redis, mock_httpx):
    """ZewmRobcoClient configured for OAuth2 mode."""
    return ZewmRobcoClient(oauth2_config)


class TestOAuth2Mode:
    """OAuth2 auth mode initialization and request flow."""

    def test_oauth2_init_sets_cfg(self, oauth2_client):
        """OAuth2 config is stored correctly after init."""
        assert oauth2_client._auth_mode == "oauth2"
        assert "token_url" in oauth2_client._oauth2_cfg
        assert oauth2_client._oauth2_cfg["client_id"] == "TEST_CLIENT_ID"

    def test_oauth2_auth_for_request_is_none(self, oauth2_client):
        """In OAuth2 mode, _get_auth_for_request returns None."""
        assert oauth2_client._get_auth_for_request() is None

    def test_oauth2_missing_token_url_raises(self, mock_redis, mock_httpx):
        """OAuth2 without token_url raises ValueError."""
    ...
    def test_oauth2_missing_client_id_raises(self, mock_redis, mock_httpx):
        """OAuth2 without client_id raises ValueError."""
        with pytest.raises(ValueError, match="client_id"):
            ZewmRobcoClient({
                "auth_mode": "oauth2",
                "oauth2": {"token_url": "https://t", "client_secret": "s"},
            })

    def test_oauth2_success_path(self, oauth2_client, mock_httpx):
        """OAuth2 mode successfully makes a request."""
        from auth import OAuth2TokenManager
        with patch.object(OAuth2TokenManager, 'get_valid_token', return_value='fake-bearer-token'):
            sap_json = {"d": {"who": "OAUTH_WHO"}}
            mock_httpx.request.return_value = _mock_response(200, sap_json)
            result = oauth2_client.get_new_robot_who("WH01", "MIR_001")
            assert result == {"who": "OAUTH_WHO"}


# ========================================================================
# 12. check_connection -- connectivity verification
# ========================================================================


class TestCheckConnection:
    """check_connection verifies SAP OData service reachability."""

    def test_check_connection_success(self, client, mock_httpx):
        """200 response from $metadata -> connected=True."""
        resp = MagicMock()
        resp.status_code = 200
        mock_httpx.get.return_value = resp
        result = client.check_connection()
        assert result["connected"] is True
        assert result["backend"] == "zewm_robco"

    def test_check_connection_failure(self, client, mock_httpx):
        """Non-200 response -> connected=False."""
        resp = MagicMock()
        resp.status_code = 500
        mock_httpx.get.return_value = resp
        result = client.check_connection()
    ...
    def test_check_connection_exception(self, client, mock_httpx):
        """Network exception -> connected=False with error."""
...
# ========================================================================
# 13. close -- resource cleanup
# ========================================================================


class TestClose:
    """close() releases Redis and httpx connections safely."""

    def test_close_without_init(self, client):
        """close() is safe to call before any lazy init."""
        client.close()

    def test_close_after_init(self, client, mock_redis):
        """close() cleans up Redis after lazy init."""
        client._ensure_redis()
        assert client._redis is not None
        client.close()
        assert client._redis is None

    def test_close_idempotent(self, client, mock_redis):
    ...
    def test_close_cleans_csrf(self, client, mock_redis):
        """close() cleans up CSRF manager."""
        client._ensure_csrf()
        assert client._csrf is not None
        client.close()
        assert client._csrf is None


# ========================================================================
# 14. Bounded retry -- 401/403/429 retries are bounded
# ========================================================================


class TestBoundedRetry:
    """Retry loop for transient failures respects _MAX_HTTP_RETRIES."""

    def test_repeated_403_stops_after_max_retries(self, client, mock_httpx):
        """403 responses stop retrying after _MAX_HTTP_RETRIES."""
        forbidden = MagicMock()
        forbidden.status_code = 403
        forbidden.text = ""
        mock_httpx.request.return_value = forbidden
        csrf_resp = MagicMock()
        csrf_resp.status_code = 200
        csrf_resp.headers = {"X-CSRF-Token": "token"}
        csrf_resp.cookies = []
        mock_httpx.get.return_value = csrf_resp
        with pytest.raises(RobcoError):
            client.get_new_robot_who("WH01", "R1")
    ...
    def test_repeated_429_stops_after_max_retries(self, client, mock_httpx):
        """429 responses stop retrying after _MAX_HTTP_RETRIES."""
    ...
    def test_401_basic_mode_not_retried(self, client, mock_httpx):
        """401 in basic auth mode is not retried."""
        unauthorized = MagicMock()
        unauthorized.status_code = 401
        unauthorized.text = ""
        unauthorized.json.return_value = {}
        mock_httpx.request.return_value = unauthorized
        with pytest.raises(RobcoError):
            client.get_new_robot_who("WH01", "R1")
...
# ========================================================================
# 15. Throttle disabled -- _last_request_time updated even when disabled
# ========================================================================


class TestThrottleDisabled:
    """_throttle updates _last_request_time even when client is disabled."""

    def test_throttle_disabled_updates_time(self, mock_redis, mock_httpx):
        """Disabled client still updates _last_request_time."""
        cfg = {"enabled": False, "base_url": "https://sap:443", "user": "U", "password": "P"}
        c = ZewmRobcoClient(cfg)
        assert c._last_request_time == 0.0
        c._throttle()
        assert c._last_request_time > 0.0

    def test_throttle_enabled_sleeps(self, mock_redis, mock_httpx):
        """Enabled client with recent request sleeps."""
        import time as _time
        cfg = {"enabled": True, "base_url": "https://sap:443", "user": "U", "password": "P", "rate_limit": 80}
        c = ZewmRobcoClient(cfg)
        c._last_request_time = _time.time()
        with patch("time.sleep") as mock_sleep:
            c._throttle()
...
# ========================================================================
# 16. auth.py -- encoding, bytes decode, secret validation
# ========================================================================


class TestAuthPy:
    """Tests for auth.py module improvements."""

    def test_read_client_secret_encoding(self, tmp_path):
        """read_client_secret reads file with UTF-8 encoding."""
        from auth import read_client_secret
        secret_file = tmp_path / "secret"
        secret_file.write_text("my_secret_value\n", encoding="utf-8")
        result = read_client_secret(str(secret_file))
        assert result == "my_secret_value"

    def test_read_client_secret_empty_raises(self, tmp_path):
        """Empty secret file raises ValueError."""
        from auth import read_client_secret
        secret_file = tmp_path / "empty"
        secret_file.write_text("", encoding="utf-8")
        with pytest.raises(ValueError, match="empty"):
            read_client_secret(str(secret_file))

    def test_read_client_secret_not_found_raises(self):
        """Missing secret file raises FileNotFoundError."""
        from auth import read_client_secret
        with pytest.raises(FileNotFoundError):
            read_client_secret("/nonexistent/path/secret")

    def test_get_token_decodes_bytes(self):
        """OAuth2TokenManager.get_token decodes bytes from Redis."""
        from auth import OAuth2TokenManager
        mock_redis = MagicMock()
        mock_redis.get.return_value = b"cached_token_bytes"
        manager = OAuth2TokenManager(
            redis_client=mock_redis,
            token_url="https://sap/token",
            client_id="id",
    ...
    def test_get_token_returns_str_unchanged(self):
        ...
        from auth import OAuth2TokenManager
        mock_redis = MagicMock()
        mock_redis.get.return_value = "cached_token_str"
        manager = OAuth2TokenManager(
            redis_client=mock_redis,
            token_url="https://sap/token",
            client_id="id",
        ...
        token = manager.get_token()
        assert token == "cached_token_str"

    def test_get_token_returns_none_when_empty(self):
        """OAuth2TokenManager.get_token returns None when no token cached."""
        from auth import OAuth2TokenManager
        mock_redis = MagicMock()
